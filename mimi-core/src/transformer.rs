...
impl Transformer {
    pub fn new(
        ...
        let inner = ProjectedTransformer::new(
            dim,
            &[dim],
            cfg,
    ...
    pub fn new_maybe_quantized(
        _batch_size: Option<usize>,
        dim: usize,
        cfg: &Config,
        vb: MaybeQuantizedVarBuilder,
    ) -> Result<Self> {
        let inner = ProjectedTransformer::new(dim, &[dim], cfg, vb)?;
        Ok(Self { inner })
    }
    ...
    pub fn reset_batch_idx(&mut self, batch_idx: usize, batch_size: usize) -> Result<()> {
        self.inner.reset_batch_idx(batch_idx, batch_size)
    }
}
